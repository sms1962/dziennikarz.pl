...
Są też gadżeciki jak "najazd" na wybrany obszar mapy. Jesli tego nie zauważyłeś, to oglądając teraz na ekranie mapę poniżej, odśwież stronę w przeglądarce. To coś podobnego jak w Google Earth, tylko nie aż tak spektakularne. Odpowiada za to funkcja *flyTo()*, której parametrami są współrzedne środka punktu na który ma nastapić "najazd" kamery i wielkość powiększenia.
Fajne, ale chyba mało przydatne :-)

```{r}

leaflet() %>% 
  addProviderTiles(provider = providers$OpenStreetMap.BlackAndWhite) %>%  
  flyTo(lng=lanKrk, lat = latKrK, zoom = 17)
```

Analogicznie jak w poprzednim przykładzie, możemy też wykonać przy pomocy funkcji flyToBounds()* najazd na zdefiniowany obszar mapy.

```{r}
leaflet() %>% 
  addProviderTiles(provider = providers$Esri.WorldImagery) %>%  
  flyToBounds(lng1 = 19.932979, lat1 = 50.063454, lng2 = 19.943965, lat2 = 50.061202)
