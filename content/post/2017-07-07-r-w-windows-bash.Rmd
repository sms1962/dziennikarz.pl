...
Najczęściej do tego typu analiz używałem R i pakietu tm. To - jak się opanuje -  dość fajne narzędzie, a przede wszystkim coś w rodzaju standardu w text mining. Co za tym idzie, sporo ludzi pisze o tym czy potrafi pomóc np. na [StackOverflow.](https://stackoverflow.com/questions/tagged/tm) Tym razem jednak postanowiłem użyć innego narzędzia: pakietu [Tidytext](https://cran.r-project.org/web/packages/tidytext/index.html). Przypomniałem sobie, że dobrych kilka miesięcy temu, a może nawet jeszcze w zeszłym roku, autorzy tekgo pakietu Julia Silge i David Robinson opublikowali książkę ["Text Mining with R. A Tidy Approach"](http://tidytextmining.com/index.html) na temat analizy tekstu, będącą niejako manualem do ich pakietu, a przy okazji fajną lekcją analizy tekstu. Jako, że przeczytałem wcześniej spore fragmenty tej ksiązki, to mniej więcej orientowałem się, że zadanie, ktore sobie postawiłem, można przy pomocy  tidytext rozwiązać w kilku linijkach kodu.

Zadanie numer 1, czyli jakie słowa padły w przemówieniu Trumpa zajęło mi jakieś circa 1 minutę :- ) bo to tyle co poniżej kodu:
...
Kod do wyodrębnienia bigramów wykorzystuje tę samą funkcję unnest_tokens() tylko nieco inne podaje się paramtery. No i w inny trochę sposób usuwa się tzw. stopwords czyli często wystepujące słowa, ktore nic nie wnoszą do analizy np. "albo", "i" etc. Wydawało się, że pójdzie jak z płatka i tak było do momentu, gdy obejrzałem na ekranie wynik. Znaczki zamiast polskich znaków!
...
Pierwsze dwa odrzuciłem, bo trochę przy tym roboty. Trzecie to trochę dla mnie niewiadoma, ale kuszące, bo na tym samym komputerze, bez żadnego ponownego uruchamiania komputera, mam pod ręką linuksa. Trochę jak na Amazon AWS, tylko na swoim kompie :-)
...
Potem już z górki, bo mamy nieco okrojony, ale jednak Ubuntu. Co prawda nie ma graficznego interfejsu (i coś mi mówi, że nie będzie :-)), a wiec nie użyjemy RStudio, no ale do incydentalnej pracy z R to nie jest jakiś specjalny problem. Co fajnego to, że z jednej strony mam linuksową przestrzeń dyskową, ale z drugiej jest też dostęp do dysku C w Windows. Dzięki temu bez żadnej gimnastyki mogę korzystać z plików w obu systemach. Instalacja R i pakietów taka sama jak w jakimkolwiek linuksie. 
